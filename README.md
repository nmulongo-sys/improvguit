...
### 2026-08-26 (nuit) — huit temps au sélecteur : un groove légal doit aussi être atteignable

Coucou et Djolé (battues 8) passaient la porte, comptaient dans « 69 grooves chargés », et
**n'apparaissaient nulle part** : le sélecteur de grooves ne liste que ceux de la mesure courante,
et la mesure ne pouvait valoir que 4, 3, 2 ou 6. Constaté en production (note 33), attendu depuis
la note 30. Ni le moteur ni le corpus n'étaient en cause — le sélecteur de mesure, seul, ne les
offrait pas.

`<option>8</option>`, et c'est tout pour l'interface : `lireBoucle`, la ligne de temps, le plafond
d'emplacements et l'ordonnanceur prenaient déjà n'importe quel nombre de temps jusqu'à 64.

⭐ **Ce qui manquait, c'est l'assertion.** « Légal » (`verifierGroove`) ne dit rien
d'« atteignable ». `battuesOffertes()` lit la liste **dans le DOM** — jamais recopiée dans le
banc, qui vieillirait sans rougir — et le banc exige désormais que chaque groove, public comme
privé, ait ses battues offertes ; la série répertoire remplace de même sa liste en dur
`[2, 3, 4, 6]` par cette lecture. Puis le chemin **réel** : un groove fabriqué à huit temps entre
par la porte, est invisible à quatre, le sélecteur de mesure reçoit `8` par son événement
`change`, la ligne de temps compte seize temps sur deux mesures, le groove paraît, est choisi, la
batterie s'arme, huit emplacements tiennent dans la mesure.

⚑ **Contre-épreuve.** Nouveau banc contre `822dc08` : **19 rouges, 0 crash** — le repli
`typeof battuesOffertes === "function"` est ce qui évite le crash qui a mordu quatre fois. Option
8 retirée du nouveau build : **9 rouges** en clone nu, **10** avec un corpus simulé portant un
groove à huit temps — la dixième est « groove privé atteignable », celle qui aurait dit Coucou.

Non traité, et dit : à quatre temps, rien n'indique qu'il existe des grooves pour d'autres
mesures. Un groove à huit temps ne se **découvre** toujours que si l'on sait régler la mesure.
Le rendu ×2 en 16 (note 30) reste l'autre voie, non ouverte.

Banc **1 460 → 1 485** en clone nu.
