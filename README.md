...
**Le corpus privé étant hors dépôt, les séries qui en dépendent sont ignorées s'il est absent** : un clone nu valide le moteur et l'interface (**1 315 assertions**, dont 66 pour la couche impro, 441 pour la boîte à rythmes et 32 pour le chargeur unique) ; avec `corpus.json` — ou, à défaut, l'ancien `repertoire.json` — posé à côté du test, la vérification fiche par fiche s'ajoute — grille lisible, longueur annoncée, gamme connue, tonalité effectivement lue, absence du titre et de l'artiste dans `index.html` — soit **1 091 assertions de plus** sur les trente-huit fiches actuelles (**2 406 en tout**, mesuré le 26 août sur les deux environnements), et chaque groove privé de `corpus.json` doit passer la porte `verifierGroove()`. Le test annonce lequel des deux régimes il a suivi.
